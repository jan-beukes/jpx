...
gui_panel_plots :: proc(panel: ^Gui_Panel, track: Gps_Track, ui_focused: ^bool) {
    ...
    **********/
	...
	for i in 0 ..< AXIS_STEPS {
		...
		draw_text(
			rl.TextFormat("%.1fkm", dist * 0.001),
			{dist_x, axis_rect.y + inner_rect.height},
			axis_font_size,
			WHITE,
		)
	}
	...
	//---Elevation---
	{
		...
		for i in 1 ..< point_count {
			...
			elev_scale :=
				(point.elevation - track.min_elevation) /
				(track.max_elevation - track.min_elevation)
			prev_elev_scale :=
				(prev_point.elevation - track.min_elevation) /
				(track.max_elevation - track.min_elevation)
	...
	value_count := 2
	top_total_text_width :=
		measure_text(distance_text, font_size) + measure_text(gain_text, font_size)
	if has_time {
		value_count += 1
	...
	total_values_width :=
		top_total_text_width +
		f32(plot_count) * (toggle_radius) +
		f32(plot_count - 1) * toggle_padding +
		f32(value_count + plot_count - 1) * value_padding

	cursor := rl.Vector2 {
		axis_rect.x + (axis_rect.width - total_values_width) * 0.5,
	...
	rl.DrawLineEx(
		{inner_rect.x, inner_rect.y + inner_rect.height},
		{inner_rect.x + inner_rect.width, inner_rect.y + inner_rect.height},
		2.0,
		BLUE,
	)
...
gui_panel_stats :: proc(panel: ^Gui_Panel, track: Gps_Track, ui_focused: ^bool) {
    ...
    *********/
	...
	rl.BeginScissorMode(
		i32(rect.x),
		i32(rect.y),
		i32(rect.width - PANEL_BORDER_THICK),
	...
	if ok {
		...
		text = rl.TextFormat(
			"%d %s %d %2d:%2d:%2d",
			date.day,
			month_names[Months(date.month - 1)],
			date.year,
			date.hour,
			date.minute,
			date.second,
...
draw_panel_handle :: proc(handle_rect: rl.Rectangle, panel: Gui_Panel) {
	...
	center := rl.Vector2 {
		handle_rect.x + (handle_rect.width * 0.5),
