...
handle_ui :: proc() {
    ...
    items := []cstring{"Osm", "Jawg Outdoors", "Mapbox Outdoors", "Satelite"}
    selected := int(req_state.tile_layer.style)
    if gui_drop_down(
        rect,
        "Map Style",
        items,
        &style_dropdown_expanded,
        &selected,
        &state.ui_is_focused,
    ) {
...
switch_tile_layer :: proc(style: Layer_Style) {
    ...
    if prev_tile_size != tile_size {
        ...
        if state.map_screen.zoom <= req_state.tile_layer.max_zoom {
            ...
            state.map_screen.center = scale_mercator(
                state.map_screen.center,
...
open_new_track :: proc(track: Gps_Track) {
    ...
    for i := 0; i < len(state.track.points); i += 1 {
        state.draw_track.coords[i] = coord_to_mercator(
            state.track.points[i].coord,
